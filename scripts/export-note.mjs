...
function findChunkOffset(buf, frameStart, frameEnd) {
  ...
  for (let skip = 9; skip <= 16; skip++) {
    ...
    if (size > 0 && size <= frameEnd - o - 6) return o;
...
function parseAseprite(buffer) {
  ...
  for (let f = 0; f < frameCount; f++) {
    ...
    offset = findChunkOffset(buffer, frameStart, frameEnd);
...
const name = 'note';
'const srcPath = path.join(SRC, name + '.aseprite');
'const outPng = path.join(OUT, name + '-0.png');
'const rawBuf = fs.readFileSync(srcPath);
'const parsed = parseAseprite(rawBuf);
'const frame0 = parsed.frames[0];
'writePng(outPng, parsed.width, parsed.height, greenify(frame0.pixels));
'let alpha = 0;
'for (let i = 3; i < frame0.pixels.length; i += 4) if (frame0.pixels[i] > 0) alpha++;
'const stat = fs.statSync(outPng);
'console.log('note-0.png size=' + stat.size);
'console.log('non-zero alpha pixels=' + alpha);
'if (alpha === 0) {
'  const hex = rawBuf.subarray(128, 200).toString('hex').match(/.{1,2}/g).join(' ');
'  fs.writeFileSync(path.join(ROOT, 'export-debug.txt'), 'bytes 128-200 hex:' + String.fromCharCode(10) + hex + String.fromCharCode(10));
'  console.log('EMPTY: wrote export-debug.txt');
'  console.log(hex);
'}
'